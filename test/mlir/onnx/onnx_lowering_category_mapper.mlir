...
// Test whether the lowering is correct for a string tensor input.
func private @test_category_mapper_string_to_int64(%arg0 : tensor<2x2x!onnx.String>) -> tensor<2x2xi64> {
  ...
  // CHECK:     [[LOAD1:%.+]] = krnl.load %arg0{{.}}[[I_0]], [[I_1]]{{.}} : memref<2x2x!krnl.string>
  // CHECK:     [[INDEX:%.+]] = "krnl.find_index"([[LOAD1]], [[G]], [[V]]) : (!krnl.string, memref<3xi32>, memref<3xi32>) -> index
  // CHECK:     [[LOAD2:%.+]] = krnl.load [[CAT_STRINGS]]{{.}}[[INDEX]]{{.}} : memref<3x!krnl.string>
  // CHECK:     [[LEN:%.+]] = "krnl.strlen"([[LOAD2]]) : (!krnl.string) -> i64
  // CHECK:     [[STRNCMP:%.+]] = "krnl.strncmp"([[LOAD1]], [[LOAD2]], [[LEN]]) : (!krnl.string, !krnl.string, i64) -> i32
  // CHECK:     [[VALID:%.+]] = arith.cmpi eq, [[STRNCMP]], [[ZERO]] : i32
  // CHECK:     scf.if [[VALID]] {
  // CHECK:     [[LOAD3:%.+]] = krnl.load [[CAT_INT64s]]{{.}}[[INDEX]]{{.}} : memref<3xi64>
  // CHECK:     krnl.store [[LOAD3]], [[ALLOCA]]{{.}}[[I_0]], [[I_1]]{{.}} : memref<2x2xi64>
  // CHECK:     } else {
  // CHECK:     krnl.store [[DEFAULT_INT64]], [[ALLOCA]]{{.}}[[I_0]], [[I_1]]{{.}} : memref<2x2xi64>
  // CHECK:     }
...
// Test whether the lowering is correct for a int64_t tensor input.
func private @test_category_mapper_int64_to_string(%arg0 : tensor<2x2xi64>) -> tensor<2x2x!onnx.String> {
  ...
  // CHECK-DAG: [[LOOP_0:%.+]]:2 = krnl.define_loops 2
  // CHECK:     krnl.iterate([[LOOP_0]]#0, [[LOOP_0]]#1) with ([[LOOP_0]]#0 -> [[I_0:%.+]] = 0 to 2, [[LOOP_0]]#1 -> [[I_1:%.+]] = 0 to 2) {  
  // CHECK:     [[LOAD1:%.+]] = krnl.load %arg0{{.}}[[I_0]], [[I_1]]{{.}} : memref<2x2xi64>
  // CHECK:     [[INDEX:%.+]] = "krnl.find_index"([[LOAD1]], [[G]], [[V]]) : (i64, memref<3xi32>, memref<3xi32>) -> index
  ...
  // CHECK:     krnl.store [[LOAD3]], [[ALLOCA]]{{.}}[[I_0]], [[I_1]]{{.}} : memref<2x2x!krnl.string>
  // CHECK:     } else {
  // CHECK:     [[LOAD4:%.+]] = krnl.load [[DEFAULT_STRING]][] : memref<!krnl.string>    
  // CHECK:     krnl.store [[LOAD4]], [[ALLOCA]]{{.}}[[I_0]], [[I_1]]{{.}} : memref<2x2x!krnl.string>
  // CHECK:     }
